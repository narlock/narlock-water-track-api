...
public interface WaterEntryRepository extends JpaRepository<WaterEntry, WaterEntryId> {
    ...
    @Query("SELECT w FROM WaterEntry w WHERE w.profileId = :profileId AND w.entryDate BETWEEN :startDate AND :endDate")
    List<WaterEntry> findByEntryDateBetween(Integer profileId, LocalDate startDate, LocalDate endDate);
}
